...
";
    ...
    const MOUSE_REPORTING_ENABLE: &[u8] =
    ...
    fn jetbrains_mono_family_available() -> bool {
        unsafe {
            ...
            let font =
                CTFontCreateWithName(name.as_concrete_TypeRef().cast(), 12.0, std::ptr::null());
            if font.is_null() {
                return false;
            }
    ...
    impl GhosttyRuntime {
        ...
        fn initialize() -> Result<Self, String> {
            ...
            let result = unsafe {
                ffi::ghostty_init(argument_pointers.len(), argument_pointers.as_mut_ptr())
            };
    ...
    fn with_palette_config<T>(
        ...
        let path =
            std::env::temp_dir().join(format!("ocherdr-ghostty-{}.conf", std::process::id()));
    ...
    impl Terminal {
        pub fn new(
            ...
            let raw = unsafe {
                ...
                config.platform_tag =
                    ffi::ghostty_platform_e_GHOSTTY_PLATFORM_METAL_EXTERNAL_LEASED;
        ...
        pub fn set_grid_size(&self, cols: u16, rows: u16) -> Result<SurfaceSize, TerminalError> {
            ...
            let changed = unsafe {
                ffi::ghostty_surface_set_grid_size(
                    self.raw(),
                    cols.max(1),
                    rows.max(1),
                    &mut resolved,
        ...
        fn rendered_newest(
            ...
            let iosurface =
                unsafe { IOSurface::wrap_under_get_rule(newest.iosurface as IOSurfaceRef) };
        ...
        pub fn send_key(
            ...
            let chord =
                (modifiers.control || modifiers.alt).then(|| chord_text(key, modifiers.shift));
        ...
        pub fn mouse_button(
            ...
            let button = match button {
                ...
                SurfaceMouseButton::Middle => {
                    ffi::ghostty_input_mouse_button_e_GHOSTTY_MOUSE_MIDDLE
                }
            };
            // SAFETY: the surface is live and called on GPUI's application thread.
            unsafe {
        ...
        pub fn select_all_visible(&self) -> bool {
            ...
            // SAFETY: the surface is live and called on GPUI's application thread.
            let ok = unsafe {
                ffi::ghostty_surface_select_viewport_rows(
                    self.raw(),
                    0,
