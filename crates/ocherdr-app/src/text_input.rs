// Adapted from ochub-ui/src/text_input.rs at ee186c95951f8cfcfd7b34e37aeced3f7bab6782.
// Copyright ochub-ui contributors; MIT OR Apache-2.0 (see NOTICE).
// Kept local so native editing can update selection directly without synthetic
// per-character actions or modifying Cargo's cached dependency source.
...
impl gpui::Global for BindingsInstalled {}
...
pub fn bind_keys(cx: &mut App) {
    cx.bind_keys([
        gpui::KeyBinding::new("backspace", Backspace, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("delete", Delete, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("left", Left, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("right", Right, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("up", Up, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("down", Down, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("shift-left", SelectLeft, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("shift-right", SelectRight, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("home", Home, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("end", End, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("enter", Newline, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("shift-enter", Newline, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("enter", FindNext, Some("OcHerdrTextInput && SearchInput")),
        gpui::KeyBinding::new(
            "shift-enter",
            FindPrevious,
            Some("OcHerdrTextInput && SearchInput"),
        ),
        gpui::KeyBinding::new("escape", CloseFind, Some("OcHerdrTextInput && SearchInput")),
        gpui::KeyBinding::new("f3", FindNext, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("shift-f3", FindPrevious, Some("OcHerdrTextInput")),
    ]);

    #[cfg(target_os = "macos")]
    cx.bind_keys([
        gpui::KeyBinding::new("cmd-a", SelectAll, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("cmd-v", Paste, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("cmd-shift-v", Paste, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("cmd-c", Copy, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("cmd-x", Cut, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("cmd-z", Undo, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("cmd-shift-z", Redo, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("cmd-f", Find, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("cmd-g", FindNext, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("cmd-shift-g", FindPrevious, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new(
            "ctrl-cmd-space",
            ShowCharacterPalette,
            Some("OcHerdrTextInput"),
        ),
    ]);

    #[cfg(any(target_os = "windows", target_os = "linux"))]
    cx.bind_keys([
        gpui::KeyBinding::new("ctrl-a", SelectAll, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("ctrl-v", Paste, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("ctrl-c", Copy, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("ctrl-x", Cut, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("ctrl-z", Undo, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("ctrl-y", Redo, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("ctrl-shift-z", Redo, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("ctrl-f", Find, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("ctrl-g", FindNext, Some("OcHerdrTextInput")),
        gpui::KeyBinding::new("ctrl-shift-g", FindPrevious, Some("OcHerdrTextInput")),
    ]);
}
...
type EnterHandler = Rc<dyn Fn(&mut Window, &mut App)>;
...
impl TextInput {
    ...
    /// Apply the canonical compact search-field treatment.
    ...
    fn newline(&mut self, _: &Newline, window: &mut Window, cx: &mut Context<Self>) {
        ...
        } else if let Some(on_enter) = self.on_enter.clone() {
            // `newline` runs while this TextInput entity owns its mutable GPUI
            // lease. Hosts commonly read the submitted input in their Enter
            // handler, so invoking it synchronously would try to lease this
            // entity again and panic. Release the current update first.
            window.defer(cx, move |window, cx| on_enter(window, cx));
        } else {
            window.play_system_bell();
        }
    }
...
impl Render for TextInput {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        #[cfg(target_os = "macos")]
        let base = base.on_key_down(cx.listener(|this, event, window, cx| {
            if this.handle_macos_editing_key(event, window, cx) {
                cx.stop_propagation();
            }
        }));
        if self.code {
            base.font_family("Menlo")
                ...
                .child(ochub_ui::scrollbar::VerticalScrollbar::new(
...
mod tests {
    use gpui::{
        Bounds, Context, Entity, Focusable, IntoElement, Render, TestAppContext, div, point,
        prelude::*, px, size,
    };

    use super::{
        CaretBlink, TextInput, accessible_value, bind_keys, closest_match, code_visible_rows,
        display_offset, extend_word_selection, find_matches, horizontal_scroll_for_caret,
        raw_offset_from_display, surrounding_word_range, utf8_offset_from_utf16,
    };
    ...
    #[gpui::test]
    fn enter_handler_can_read_the_input_after_its_update(cx: &mut TestAppContext) {
        cx.update(bind_keys);
        let (probe, cx) = cx.add_window_view(|window, cx| {
            let input = cx.new(|cx| TextInput::new(cx, "name").with_content("alpha"));
            let weak_probe = cx.weak_entity();
            input.update(cx, |input, _| {
                input.set_on_enter(move |_window, cx| {
                    weak_probe
                        .update(cx, |probe: &mut EnterProbe, cx| {
                            assert_eq!(probe.input.read(cx).content().as_ref(), "alpha");
                            probe.submitted = true;
                        })
                        .expect("enter probe remains alive");
                });
            });
            input.read(cx).focus_handle(cx).focus(window, cx);
            EnterProbe {
        ...
        cx.run_until_parked();
        probe.read_with(cx, |probe, _| assert!(probe.submitted));
    }
